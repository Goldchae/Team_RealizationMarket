...
public class jec_Pick_Controller : MonoBehaviour
{
    ...
    {
        int clickCounter = UI.GetComponent<jec_UI_Controller>().GetPickCounts();
        clickCounter++;
        print($"{clickCounter} 개의 클론을 클릭했습니다.");
        Destroy(Clone);

        // Step04 (UI에 내용을 표시하는 스크립트 호출)
        UI.GetComponent<jec_UI_Controller>().Display_PickCounts(clickCounter);
    }
}
