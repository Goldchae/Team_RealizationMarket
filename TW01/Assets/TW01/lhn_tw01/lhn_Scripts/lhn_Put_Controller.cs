...
public class lhn_Put_Controller : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            int pickCounts = UI.GetComponent<lhn_UI_Controller>().GetPickCounts();
            if (pickCounts > 0)
            {
                Throw();
                UI.GetComponent<lhn_UI_Controller>().Decrease_PickCounts();
            }
        }
    }
